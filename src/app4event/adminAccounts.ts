...
const config = {
  ...
  oAuth2ClientId:
    '162840149532-3ol8e0s2et2afhvdnn25mfp0h908tvpc.apps.googleusercontent.com',
        ...
        try {
          ...
          if (requestUrl.includes('/oauth2callback')) {
            const qs = new url.URL(
              requestUrl,
              `http://localhost:${config.oAuth2ServerPort}`
            ).searchParams
            ...
            void oAuth2Client.getToken(code).then(r => {
...
export const makeAdmin = async (
  ...
  echo(
    `Making admin of user email=${username}, password=${password} in project ${project}`
  )
...
export const setPassword = async (
  ...
  if (!existingUser) {
    ...
    echo(
      `Run \`npm a4e-createAdmin ${project} ${username} ${password}\` to register the user`
    )
    return
  }
