...
export { addItems, probe, sanitizeCustomFields, sanitizeLinks, SavedState }
...
export const createImporter = async (settings: Settings) => {
  ...
  const i = {
    ...
    usePerformerNameAsSessionName:
      settings.usePerformerNameAsSessionName ?? false,
...
export const createImporterFromState = async (
  settings: Settings,
...
export const deleteUnreferenced = async (importer: EventImporter) => {
  ...
  async function getKeepIds(ent: Item['type']) {
    const downloadedIds: string[] =
      (await importer.store.get(`${ent}-ids`)) ?? []
   ...
   */
  async function pruneLanguagesCollection(
    ent: Exclude<Item['type'], 'language'>
  ) {
    await Promise.all(
      importer.settings.languages.map(async language => {
        const collectionName = getEntityCollection(ent)({
          lang: language,
        } as any)
        ...
        deleteIds.forEach(_id => {
          importer.warnings.push(
            errors.createImportError(importer, errors.DELETED_DATABASE_ITEM)
          )
        })
...
const saveLanguages = async (importer: EventImporter) => {
  const languages: Language[] = importer.settings.languages.map(
    languageCode => {
      return {
        id: languageCode,
        type: 'language',
        ...
        data: {
          ...
          name: ISO6391.default.getNativeName(languageCode) || languageCode,
          id: languageCode,
        },
      }
    }
...
const saveDays = async (importer: EventImporter) => {
  ...
  await util.settle(
    validated.results.map(item =>
      firestore.save(
        importer.firestore,
        firestore.path['/languages/{lang}/days/{id}']({
          lang: item.language,
          id: item.id,
        }),
        firestore.convertFirstoreKeys(item.data, {
          dates: ['timeFrom', 'timeTo'],
        })
      )
    )
  )
...
const saveVenueCategories = async (importer: EventImporter) => {
  ...
  const constructed = await constructItems(
    importer,
    'venueCategory',
    (item, meta) => {
      return {
  ...
  await util.settle(
    validated.results.map(item =>
      firestore.save(
        importer.firestore,
        firestore.path['/languages/{lang}/venueCategories/{id}']({
          lang: item.language,
          id: item.id,
        }),
        item.data
...
const saveVenues = async (importer: EventImporter) => {
  ...
  const constructed = await constructItems(
    ...
    async (item, meta) => {
      ...
      const categories = await populateId(
        importer,
        'venueCategory',
        item.data.categoryIds,
        meta.languageCode
      )
      if (!categories?.length) {
        importer.warnings.push(
          errors.createImportError(importer, errors.MISSING_VENUE_CATEGORIES, {
            item,
          })
        )
      }
  ...
  await util.settle(
    validated.results.map(item =>
      firestore.save(
        importer.firestore,
        firestore.path['/languages/{lang}/venues/{id}']({
          lang: item.language,
          id: item.id,
        }),
...
const savePerformers = async (importer: EventImporter) => {
  ...
  const constructed = await constructItems(
    ...
    'performer',
    async (item, meta) => {
      const referencedSessionIds: Array<Session['data']['id']> =
        (await importer.store.get(`performer2sessions:${meta.id}`)) ?? []
      const referencedVenueIds: Array<NonNullable<Session['data']['venueId']>> =
        (await importer.store.get(`performer2venues:${meta.id}`)) ?? []
      const sessions = await populateId(
        importer,
        'session',
      ...
      return {
        ...
        data: {
          ...
          description: item.data.description
            ? util.stripHtml(item.data.description).trim()
  ...
  await util.settle(
    validated.results.map(item =>
      firestore.save(
        importer.firestore,
        firestore.path['/languages/{lang}/performers/{id}']({
          lang: item.language,
          id: item.id,
        }),
        item.data
...
const saveSessions = async (importer: EventImporter) => {
  ...
  const constructed = await constructItems(
    ...
    'session',
    async (item, meta) => {
      const performers = await populateId(
        importer,
        'performer',
        item.data.performerIds,
        meta.languageCode
      )
      const venue = await populateId(
        importer,
        'venue',
        item?.data.venueId,
        meta.languageCode
      ...
      const subsessionIds =
        (
          await populateId(
            importer,
            'session',
            item?.data.subsessionIds,
            meta.languageCode
          )
        )?.map(x => x.id) ?? []
      const parentIds: Array<Item['id']> =
        (await importer.store.get(`session2parent:${item.id}`)) ?? []
      const parents = await populateId(
        importer,
        'session',
        parentIds,
        meta.languageCode
  ...
  await util.settle(
    validated.results.map(item =>
      firestore.save(
        importer.firestore,
        firestore.path['/languages/{lang}/sessions/{id}']({
          lang: item.language,
          id: item.id,
        }),
        firestore.convertFirstoreKeys(item.data, {
...
const saveGroups = async (importer: EventImporter) => {
  ...
  const constructedAll = await constructItems(
    importer,
    'group',
    async (item, meta) => {
      const sessionIds: Array<Item['id']> =
        (await importer.store.get(`group2sessions:${item.id}`)) ?? []
      const sessions = await populateId(
        importer,
        'session',
        sessionIds,
      ...
      const performerIds: Array<Item['id']> =
        (await importer.store.get(`group2performers:${item.id}`)) ?? []
      const performers = await populateId(
        importer,
        'performer',
        performerIds,
  ...
  const constructed = {
    ...constructedAll,
    results: constructedAll.results.flatMap(x => x),
  }
  const validated = await validateItems(importer, constructed.results)
  await util.settle(
    validated.results.map(item =>
      firestore.save(
        importer.firestore,
        firestore.path['/languages/{lang}/groups/{id}']({
          lang: item.language,
          id: item.id,
        }),
        firestore.convertFirstoreKeys(item.data, {})
    ...
    constructedItems.results
      ...
      .map(item => {
        return firestore.save(
          importer.firestore,
          firestore.path['/languages/{lang}/groups/{groupId}/items/{id}']({
            lang: item.language,
            groupId: item.groupId,
            id: item.id,
          }),
          firestore.convertFirstoreKeys(item.data, {
            dates: ['timeFrom', 'timeTo'] as any[] /* session keys */,
          })
        )
      })
  )
...
const saveHighlights = async (importer: EventImporter) => {
  ...
  await util.settle(
    validated.results.map(item =>
      firestore.save(
        importer.firestore,
        firestore.path['/languages/{lang}/highlights/{id}']({
          lang: item.language,
          id: item.id,
        }),
        firestore.convertFirstoreKeys(item.data, {})
...
export const startLoading = async (
  importer: EventImporter,
  load: (importer: EventImporter) => Promise<void>
) => {
  const clearDataLoadProgress = startDataLoadProgress(importer)
  try {
    await load(importer)
  } catch (error) {
    importer.errors.push(
      errors.createImportError(importer, errors.LOADING_DATA_FAILED)
    )
